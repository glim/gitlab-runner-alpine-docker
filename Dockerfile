...
RUN curl -s https://get.docker.com/builds/Linux/x86_64/docker-17.03.0-ce.tgz | tar -xzf - docker/docker -O  > /usr/bin/docker && chmod +x /usr/bin/docker
...
RUN adduser -h /home/gitlab-runner -s /bin/ash -D gitlab-runner ping
VOLUME /etc/gitlab-runner
COPY config.toml /etc/gitlab-runner/config.toml
CMD gitlab-ci-multi-runner
